<template>
        ...
        <Breadcrumb :style="{margin: '20px 0'}">
                    <BreadcrumbItem><router-link :to="{ name: 'index', params: {} }">首页</router-link></BreadcrumbItem>
    ...
    computed: {
        ...
        breadData() {
            ...
            switch (name) {
                ...
                case 'subject':
                    d = [{name: category.name, id: category.id, type:'category'}, {name: subject.name, id: subject.id, type:'subject'},];
                    break;
                case 'play':
                    d = [{name: category.name, id: category.id, type:'category'}, {name: subject.name, id: subject.id, type:'subject'}, {name: video.name, id: video.id},];
                    break;
